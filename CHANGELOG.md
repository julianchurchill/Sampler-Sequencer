...
### Fixed
- Samples on the same track no longer overlap when re-triggered; a generation counter ensures only the latest trigger's play() call executes, stopping any in-flight playback from a previous trigger
