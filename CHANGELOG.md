...
### Added
- Custom time signatures: tap the step count label in the transport bar to
  choose from 2/4, 3/4, 4/4, 5/4, 6/8, 7/8, 9/8, or 12/8.  The pad grid
  resizes to reflect the new bar length (8–24 steps), visual beat separators
  update to match the grouping of the new signature (quarter-note groups for
  `/4` time, dotted-quarter groups for compound `/8` time), and the selected
  signature is persisted across app restarts.  Existing programmed steps are
  preserved up to the new bar length.
