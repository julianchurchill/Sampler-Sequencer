...
### Fixed
- `_save()` in `SequencerModel` now catches errors from `SharedPreferences.getInstance()`
  instead of silently swallowing them. Persistence failures (e.g. storage full, permission
  revoked) are now logged via `debugPrint`.
