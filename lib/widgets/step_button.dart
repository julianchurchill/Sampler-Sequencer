...
class StepButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // isCurrent is driven by ValueListenableBuilder rather than context.select
    // so that only the two buttons whose highlighted state changes rebuild on
    // each sequencer tick (instead of triggering all 64 context.select
    // evaluations via notifyListeners on every tick).
    return ValueListenableBuilder<int>(
      valueListenable: context.read<SequencerModel>().currentStepNotifier,
