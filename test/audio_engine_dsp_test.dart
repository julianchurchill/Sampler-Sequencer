...
void main() {
  ...
  group('drum generators', () {
    ...
    test('two simultaneous Kick808 streams at adjacent 120-BPM steps do not clip', () {
      ...
      // produce overlapping streams in the Android audio mixer. If their
      // combined amplitude exceeds 1.0 at any point the mixer hard-clips,
      // producing an audible crack on the 2nd hit. This test verifies the sum
      // stays ≤ 1.0.
      final buf = generateKick808(_kSampleRate);
      ...
      for (int i = 0; i < buf.length; i++) {
        ...
        expect(combined, lessThanOrEqualTo(1.0),
            reason: 'generateKick808: two streams offset by 125 ms clip at sample $i '
                '(s1=$s1, s2=$s2, sum=${s1 + s2}). Hard PCM clipping in the Android '
                'audio mixer causes an audible click on the 2nd adjacent kick hit.');
      }
    });
  });
}
